  ...
  return (
    <>
      <nav
        ...
        style={{ background: scrolled ? 'rgba(10,26,47,0.97)' : 'transparent' }}
      >
        <div className="w-full px-4 h-16 flex items-center justify-between gap-3">
          ...
          <div className="hidden md:flex items-center gap-1.5">
            ...
            `}</style>
            <img src="https://flagcdn.com/w40/pe.png" alt="Peru" className="w-9 h-6 rounded-sm object-cover shadow-sm" style={{ animation: 'wave-pe 1.1s ease-in-out infinite', transformOrigin: 'left center', filter: 'saturate(1.9) brightness(1.15) contrast(1.1)' }} />
          ...
          <div ref={langRef} className="relative hidden md:block">
            ...
            {langOpen && (
              <div className="absolute right-0 top-full mt-2 border border-gold/20 rounded-lg overflow-hidden shadow-xl min-w-[140px]" style={{ background: "#0A1A2F" }}>
                {LANGUAGES.map((lang) => (
                  <button
                    key={lang.code}
          ...
          <div className="md:hidden flex items-center gap-2">
            <div className="flex items-center gap-1">
              <img src="https://flagcdn.com/w40/pe.png" alt="Peru" className="w-6 h-4 rounded-sm object-cover" style={{ animation: 'wave-pe 1.1s ease-in-out infinite', transformOrigin: 'left center', filter: 'saturate(1.9) brightness(1.15) contrast(1.1)' }} />
            ...
            <div ref={mobileLangRef} className="relative">
              ...
              {mobileLangOpen && (
                <div className="absolute right-0 top-full mt-2 border border-gold/20 rounded-xl overflow-hidden z-[60]" style={{ minWidth: 130, boxShadow: '0 12px 40px rgba(0,0,0,0.5)', background: "#0A1A2F" }}>
                  {LANGUAGES.map((lang) => (
                    <button
                      key={lang.code}
                      onClick={() => changeLanguage(lang.code)}
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-300 md:hidden ${mobOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}>
        ...
        <div className={`absolute top-0 right-0 w-72 h-full transition-transform duration-300 ${mobOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col p-8`} style={{ background: "#0A1A2F" }}>
